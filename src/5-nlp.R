...
library(tidytext)
pedidos_cgu %>%
  select(id_pedido,orgao = orgaodestinatario, resposta) %>% 
  sample_n(500) %>% 
...
stopwords_tm <- tibble(word = limpando_texto(tm::stopwords("pt")), lexicon = "tm")
...
stopwords_lai <- tibble(word = stopwords_lai, lexicon = "lai")
