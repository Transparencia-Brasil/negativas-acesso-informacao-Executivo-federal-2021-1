...
cores_detecta_lgpd <- c(
  "Instância inexistente" = cores_aep[["cinza"]],
...
recs %>%  
  ...
  mutate(onde = ordered(onde, levels = ord_interacao),
         usa = case_when(
           ...
           TRUE ~ "Instância inexistente"
  )) %>% 
  ggplot(aes(x = onde, y = combinacao, fill = usa)) +
  ...
  labs(
    ...
    subtitle = "Células coloridas são as interações de um pedido,\n cinza a interação não existe",
    fill = NULL,
    x = NULL,
    y = NULL
  ) +
