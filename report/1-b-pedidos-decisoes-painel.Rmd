...
pedidos_cgu <- "dados/load/rds/base_cgu.rds" %>%
  here() %>%
  readRDS() %>%
  pluck("pedidos") %>%
  janitor::clean_names() %>% 
...
p1 <- decisao %>%
  ...
  ggforce::geom_mark_circle(
    aes(filter = data_resposta == ymd("2018-09-01") & decisao == "Acesso Concedido",
...
```{r}
decisao %>%
  mutate(mesmo_periodo = month(data_resposta) <= 9) %>%
  ...
  scale_x_date(
    breaks = ymd(c(glue("{2012:2021}-01-01"), glue("{2012:2021}-09-01"))),
    date_labels = "%b"#,
    #expand = c(.1, .1)
    ) +
  theme_minimal() +
