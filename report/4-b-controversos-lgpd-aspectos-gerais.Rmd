...
d <- lgpd %>%
  filter(usa_lgpd) %>% 
  filter(data_registro != min(data_registro)) %>%
  filter(data_registro != min(data_registro)) %>%
  filter(data_registro != min(data_registro)) %>% 
  filter(data_registro != min(data_registro)) %>%
  filter(data_registro == min(data_registro)) %>% 
...
pedidos_cgu %>%
  ...
  mutate(str_glue("http://www.consultaesic.cgu.gov.br/busca/_layouts/15/DetalhePedido/DetalhePedido.aspx?nup={protocolo_pedido}"))
