...
controversos_lgpd <- "dados/load/rds/controversos_lgpd.rds" %>% here() %>% readRDS()
```
...
controversos_lgpd %>%
  count(ano = year(data_resposta), sort = T) %>% 
...
controv %>%  
  count(ano = year(data_resposta), decisao, name = "n_ano") %>% 
  ...
  geom_col(color = "gray20", position = "dodge") +
  geom_label(
    aes(label = if_else(n_ano > 0, n_ano, NA_real_)),
    show.legend = F,
  ...
  scale_color_manual(values = aux2) +
  theme_minimal() +
  labs(
    title = "Pedidos com menções à LGPD na resposta",
    subtitle = "Por decisão de acesso",
    fill = "Decisão",
    color = "Decisão",
    y = "Quantidade",
    x = NULL
  ) +
  theme(legend.text = element_text(size = 8, margin = margin(5, 0, 5, 0, "pt")))
```
