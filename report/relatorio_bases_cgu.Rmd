...
* O código para extração e tratamento dos arquivos `xml` estão disponibilizados [neste script do Jupyter Notebook.]()
...
```{r echo = FALSE}
periodo <- read_csv("../data/pedidos_cgu.csv", col_types = cols_only("DataRegistro" = col_character())) %>% 
  mutate(DataRegistro = dmy_hms(DataRegistro) %>% as_date()) %>% 
  group_by() %>%
  summarise(inicio = min(DataRegistro), fim = max(DataRegistro))
```

As bases de dados vão de `r periodo$inicio` até `r periodo$fim`. Este período cobre o final do segundo mandato de Dilma Rousseff, o mandato de Michel Temer e o mandato de Jair Bolsonaro até `r month(Sys.Date(), label = TRUE, abbr = FALSE)` de `r year(Sys.Date())`.
