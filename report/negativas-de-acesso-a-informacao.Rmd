...
* Resultados relacionados aos termos controvérsos (em breve).
...
pedidos_por_mes %>% 
  ...
  #scale_y_continuous(breaks = seq(0, 10, 2), limits = c(0, 11.5), labels = seq(0, 10, 2)) +
...
p3 <- indice_respostas_concedidas %>%
  ...
  geom_text(aes(x = data_registro, y = indice_acessos_concedidos, 
                label = scales::percent(indice_acessos_concedidos, accuracy = 1.0)), vjust = -1, size = 3) +
...
##### Frequência das classificações de atendimento atribbuídas pelos órgãos do Executivo federal - por mês e por governo
...
comb + 
  plot_layout(guides = "collect", widths = c(2.7, 1))
```
...
p2 <- controversos2 %>%
  ...
  labs(
    x = NULL,
    y = NULL,
    subtitle = "Percentual de acessos negados (%)",
    color = NULL,
    fill = NULL
  ) +
  facet_wrap(~ reorder(controversos, -perc), scales = 'free', ncol = 1)
